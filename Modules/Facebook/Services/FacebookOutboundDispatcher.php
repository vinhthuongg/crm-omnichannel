...
class FacebookOutboundDispatcher
{
    ...
    {
        try { return $this->payload($recipient, $content, $attachments, $token); }
        catch (RequestException $exception) {
            if (! $this->isControlError($exception)) throw $exception;
            ...
            if (! $this->facebook->takeThreadControl($recipient, $token)) throw $exception;
    ...
    private function payload(string $recipient, string $content, array $attachments, ?string $token): array
    {
        ...
        foreach ($attachments as $index => $attachment) {
            if (($attachment['type'] ?? '') === 'quick_reply') continue;
            if (empty($attachment['facebook_attachment_id']) && empty($attachment['url']) && empty($attachment['path'])) continue;
            try {
                ...
                try { $response = $this->facebook->sendAttachment($recipient, (string) ($attachment['url'] ?? ''), $type, $token); }
                catch (RequestException $exception) {
                    if (! $this->isUploadError($exception)) throw $exception;
                    $tmp = $this->download($attachment);
                    try { $response = $this->facebook->sendLocalAttachment($recipient, $tmp, $type, (string) ($attachment['mime_type'] ?? ''), (string) ($attachment['name'] ?? basename($tmp)), $token); }
                    finally { @unlink($tmp); }
                    $this->captureId($sent, $index, $response);
                }
        ...
        if ($sent) $response['_sent_attachments'] = $sent;
        if ($failed) $response['_failed_attachments'] = $failed;
        return $response;
    }
    ...
    {
        if (! empty($response['facebook_attachment_id'])) $sent[$index] = ['facebook_attachment_id' => (string) $response['facebook_attachment_id']];
    }

    /** Trích xuất danh sách quick reply từ metadata của tệp đính kèm. */
    private function quickReplies(array $attachments): array
    {
        foreach ($attachments as $item) if (($item['type'] ?? '') === 'quick_reply') return array_values((array) ($item['quick_replies'] ?? []));
        return [];
    }
    ...
    private function download(array $attachment): string
    {
        ...
        if ($url === '') throw new RuntimeException('Attachment URL is empty.');
        $response = Http::connectTimeout(10)->timeout(45)->get($url)->throw();
        $tmp = tempnam(sys_get_temp_dir(), 'crm_message_attachment_');
        if ($tmp === false) throw new RuntimeException('Cannot create temporary file for attachment upload.');
    ...
    {
        return match (true) { str_starts_with($mime, 'image/') => 'image', str_starts_with($mime, 'video/') => 'video',
    ...
    {
        $error = (array) Arr::get((array) $e->response->json(), 'error', []); $message = mb_strtolower((string) Arr::get($error, 'message', ''));
    ...
    private function exceptionContext(\Throwable $e): array
    {
        ...
        if ($e instanceof RequestException && $e->response) { $context['status'] = $e->response->status(); $context['body'] = mb_substr($e->response->body(), 0, 2000); }
        return $context;
    }
}
